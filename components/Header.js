...
function Header({placeholder}) {
    ...
    return(
        <header className={'sticky top-0 z-50 flex sm:grid sm:grid-cols-3 items-center justify-between bg-[#f7f7f7] shadow-md p-5 md:px-10 w-[100%] '}>
        {/*Left */}
        ...
        {/*Middle*/}
            <div style={{display: showSearch && 'flex'}}
                 className={'flex hidden absolute h-3/5 w-4/5 sm:h-full shadow-lg border-1 sm:relative bg-white w-full  sm:inline-flex items-center md:border-2 rounded-full py-2 md:shadow-lg text-sm text-gray-900'}>
                { showCross && (
                    <div onClick={()=>{
                        setShowSearch(false);
                        setShowCross(false);
        ...
        {/*Right*/}
            <div className={'flex hidden sm:inline-flex items-center space-x-4 justify-end text-gray-500'}>
            ...
            {searchInput && (
                <div className={'absolute  md:w-fit bottom-0 translate-y-full md:translate-y-0 bg-white md:relative md:flex md:flex-col md:col-span-3 mx-auto mt-5 px-4 py-8'}>
